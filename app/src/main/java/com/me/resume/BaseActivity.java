...
//	}
	...
	protected void userRegisterType(EditText editText){
		...
		if (showRegType) {
			...
			img_regtype_left = regType.getDrawable(R.mipmap.icon_email);
			editText.setInputType(InputType.TYPE_TEXT_VARIATION_EMAIL_ADDRESS);
		}else{
			editText.setHint(getStrValue(R.string.register_input2_hint));
			img_regtype_left = regType.getDrawable(R.mipmap.icon_phone);
		...
		commapBaseAdapter = new CommForMapBaseAdapter(self,map,R.layout.home_share_item,"id") {
			...
			public void convert(final ViewHolder holder, List<String> item, final int position) {
				...
				if (RegexUtil.checkNotNull(avatorStr)) {
					...
					holder.setImageResource(R.id.share_usernameavator, R.mipmap.user_default_avatar);
				}
				...
				if (commMapArray == null) {
					holder.setImageResource(R.id.share_collection, R.mipmap.icon_collection_nor);
				}else{
					if (!MyApplication.USERID.equals("0")) {
						holder.setImageResource(R.id.share_collection, R.mipmap.icon_collection_sel);
					}
				}
